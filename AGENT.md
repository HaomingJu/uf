...
- `Ctrl+B` toggles the `Config` tab. Keep Config tab content searchable through the existing search box and organized as level 1 / level 2 / level 3 menu groups, where level 1 is the source or app area such as Browser/GitHub/GitLab/DockerHub/System, and level 2 is the feature such as Source/Auth/API/Refresh. Top-level groups with child rows should render with a `▼` prefix, and child config rows should use only four leading spaces for indentation. Config rows can be selected and toggled or edited in the current UI state. Never render raw token values; show token status as present or missing.
